(function (global, factory) {
	...
	(global = global || self, (global.CrystalNavigation = global.CrystalNavigation || {}, global.CrystalNavigation.umd = factory()));
	...
	var getElement = Crystal.getElement;
	var Knockout = Crystal.knockout;
	...
	var defined$1 = Crystal.defined;
	var DeveloperError = Crystal.DeveloperError;
	var EllipsoidGeodesic = Crystal.EllipsoidGeodesic;
	var Cartesian2 = Crystal.Cartesian2;
	var getTimestamp = Crystal.getTimestamp;
	var EventHelper = Crystal.EventHelper;
	var Knockout$1 = Crystal.knockout;
	  ...
	  function addUpdateSubscription() {
	    if (defined$1(that.terria)) {
	      ...
	      that._removeSubscription = scene.postRender.addEventListener(function () {
	        updateDistanceLegendCrystal(this, scene);
	      }, that);
	    }
	  }
	...
	function updateDistanceLegendCrystal(viewModel, scene) {
	...
	var defined$2 = Crystal.defined;
	var defineProperties = Object.defineProperties;
	var DeveloperError$1 = Crystal.DeveloperError;
	var Knockout$2 = Crystal.knockout;
	/**
	...
	var defined$3 = Crystal.defined;
	var Camera = Crystal.Camera;
	var Rectangle = Crystal.Rectangle;
	var Cartographic = Crystal.Cartographic;
	/**
	      ...
	      if (this.terria.options.defaultResetView && this.terria.options.defaultResetView instanceof Cartographic) {
	        ...
	        try {
	          ...
	          camera.flyTo({
	            ...
	            orientation: {
	              heading: Crystal.Math.toRadians(5.729578)
	            }
	          });
	        } catch (e) {
	          console.log('Crystal-navigation/ResetViewNavigationControl:   options.defaultResetView Crystal rectangle is  invalid!');
	        }
	      }
	...
	var defined$4 = Crystal.defined;
	var Ray = Crystal.Ray;
	var Cartesian3 = Crystal.Cartesian3;
	var Cartographic$1 = Crystal.Cartographic;
	var ReferenceFrame = Crystal.ReferenceFrame;
	var SceneMode = Crystal.SceneMode;
	var Utils = {};
	...
	var defined$5 = Crystal.defined;
	var Ray$1 = Crystal.Ray;
	var IntersectionTests = Crystal.IntersectionTests;
	var Cartesian3$1 = Crystal.Cartesian3;
	var SceneMode$1 = Crystal.SceneMode;
	/**
	 * The model for a zoom in control in the navigation control tool bar
	 *
	...
	var defined$6 = Crystal.defined;
	var CrystalMath = Crystal.Math;
	var getTimestamp$1 = Crystal.getTimestamp;
	var EventHelper$1 = Crystal.EventHelper;
	var Transforms = Crystal.Transforms;
	var SceneMode$2 = Crystal.SceneMode;
	var Cartesian2$1 = Crystal.Cartesian2;
	var Cartesian3$2 = Crystal.Cartesian3;
	var Matrix4 = Crystal.Matrix4;
	var BoundingSphere = Crystal.BoundingSphere;
	var HeadingPitchRange = Crystal.HeadingPitchRange;
	var Knockout$3 = Crystal.knockout;
	...
	NavigationViewModel.prototype.destroy = function () {
	  this.eventHelper.removeAll(); // loadView(require('fs').readFileSync(baseURLEmpCrystal + 'js-lib/terrajs/lib/Views/Navigation.html', 'utf8'), container, this);
	};
	...
	NavigationViewModel.prototype.show = function (container) {
	  ...
	  if (this.enableZoomControls && this.enableCompass) {
	    testing = '<div class="compass" title="拖动外圈:旋转地球;拖动内部陀螺仪:自由轨道;双击:置北" data-bind="visible: showCompass, event: { mousedown: handleMouseDown, dblclick: handleDoubleClick }">' + '<div class="compass-outer-ring-background"></div>' + ' <div class="compass-rotation-marker" data-bind="visible: isOrbiting, style: { transform: \'rotate(-\' + orbitCursorAngle + \'rad)\', \'-webkit-transform\': \'rotate(-\' + orbitCursorAngle + \'rad)\', opacity: orbitCursorOpacity }, crystalSvgPath: { path: svgCompassRotationMarker, width: 145, height: 145 }"></div>' + ' <div class="compass-outer-ring" title="" data-bind="style: { transform: \'rotate(-\' + heading + \'rad)\', \'-webkit-transform\': \'rotate(-\' + heading + \'rad)\' }, crystalSvgPath: { path: svgCompassOuterRing, width: 145, height: 145 }"></div>' + ' <div class="compass-gyro-background"></div>' + ' <div class="compass-gyro" data-bind="crystalSvgPath: { path: svgCompassGyro, width: 145, height: 145 }, css: { \'compass-gyro-active\': isOrbiting }"></div>' + '</div>' + '<div class="navigation-controls">' + '<!-- ko foreach: controls -->' + '<div data-bind="click: activate, attr: { title: $data.name }, css: $root.isLastControl($data) ? \'navigation-control-last\' : \'navigation-control\' ">' + '   <!-- ko if: $data.hasText -->' + '   <div data-bind="text: $data.text, css: $data.isActive ?  \'navigation-control-icon-active \' + $data.cssClass : $data.cssClass"></div>' + '   <!-- /ko -->' + '  <!-- ko ifnot: $data.hasText -->' + '  <div data-bind="crystalSvgPath: { path: $data.svgIcon, width: $data.svgWidth, height: $data.svgHeight }, css: $data.isActive ?  \'navigation-control-icon-active \' + $data.cssClass : $data.cssClass"></div>' + '  <!-- /ko -->' + ' </div>' + ' <!-- /ko -->' + '</div>';
	  ...
	  // loadView(require('fs').readFileSync(baseURLEmpCrystal + 'js-lib/terrajs/lib/Views/Navigation.html', 'utf8'), container, this);
	};
	/**
	...
	NavigationViewModel.prototype.handleDoubleClick = function (viewModel, e) {
	  ...
	  camera.flyToBoundingSphere(focusBoundingSphere, {
	    ...
	    CrystalMath.PI_OVER_TWO - Cartesian3$2.angleBetween(surfaceNormal, camera.directionWC), // distanceToBoundingSphere returns wrong values when in 2D or Columbus view so do not use
	...
	function orbit(viewModel, compassElement, cursorVector) {
	  ...
	  viewModel.orbitTickFunction = function (e) {
	    ...
	    var angle = viewModel.orbitCursorAngle + CrystalMath.PI_OVER_TWO;
	  ...
	  function updateAngleAndOpacity(vector, compassWidth) {
	    ...
	    viewModel.orbitCursorAngle = CrystalMath.zeroToTwoPi(angle - CrystalMath.PI_OVER_TWO);
	...
	function rotate(viewModel, compassElement, cursorVector) {
	  ...
	  viewModel.rotateMouseMoveFunction = function (e) {
	    ...
	    var newCameraAngle = CrystalMath.zeroToTwoPi(viewModel.rotateInitialCameraAngle - angleDifference);
	...
	var defined$7 = Crystal.defined;
	var CrystalEvent = Crystal.Event;
	var Knockout$4 = Crystal.knockout;
	var DeveloperError$2 = Crystal.DeveloperError;
	/**
	 * @alias CrystalNavigation
	 * @constructor
	 *
	 * @param {Viewer|CrystalWidget} viewerCrystalWidget The Viewer or CrystalWidget instance
	 */

	var CrystalNavigation = function CrystalNavigation(viewerCrystalWidget) {
	  initialize.apply(this, arguments);
	  this._onDestroyListeners = [];
	};

	CrystalNavigation.prototype.distanceLegendViewModel = undefined;
	CrystalNavigation.prototype.navigationViewModel = undefined;
	CrystalNavigation.prototype.navigationDiv = undefined;
	CrystalNavigation.prototype.distanceLegendDiv = undefined;
	CrystalNavigation.prototype.terria = undefined;
	CrystalNavigation.prototype.container = undefined;
	CrystalNavigation.prototype._onDestroyListeners = undefined;
	CrystalNavigation.prototype._navigationLocked = false;

	CrystalNavigation.prototype.setNavigationLocked = function (locked) {
	  this._navigationLocked = locked;
	  this.navigationViewModel.setNavigationLocked(this._navigationLocked);
	};

	CrystalNavigation.prototype.getNavigationLocked = function () {
	  return this._navigationLocked;
	};

	CrystalNavigation.prototype.destroy = function () {
	...
	CrystalNavigation.prototype.addOnDestroyListener = function (callback) {
	...
	function initialize(viewerCrystalWidget, options) {
	  if (!defined$7(viewerCrystalWidget)) {
	    throw new DeveloperError$2('CrystalWidget or Viewer is required.');
	  }

	  var crystalWidget = defined$7(viewerCrystalWidget.crystalWidget) ? viewerCrystalWidget.crystalWidget : viewerCrystalWidget;
	  var container = document.createElement('div');
	  container.className = 'crystal-widget-crystalNavigationContainer';
	  crystalWidget.container.appendChild(container);
	  this.terria = viewerCrystalWidget;
	  this.terria.options = defined$7(options) ? options : {};
	  this.terria.afterWidgetChanged = new CrystalEvent();
	  this.terria.beforeWidgetChanged = new CrystalEvent();
	...
	return CrystalNavigation;

})));
//# sourceMappingURL=CrystalNavigation.umd.js.map
