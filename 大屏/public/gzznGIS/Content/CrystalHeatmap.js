(function (window) {
	...
	function define_CrystalHeatmap() {
		var CrystalHeatmap = {
			defaults: {
		...
		/*  Create a CrystalHeatmap instance
		 *
		 *  cesium:  the CrystalWidget or Viewer instance
		 ...
		 */
		CrystalHeatmap.create = function (cesium, bb, options) {
			var instance = new CHInstance(cesium, bb, options);
			return instance;
		};

		CrystalHeatmap._getContainer = function (width, height, id) {
		...
		CrystalHeatmap._getImageryProvider = function (instance) {
			//var n = (new Date()).getTime();
			var d = instance._heatmap.getDataURL();
			//console.log("Create data URL: " + ((new Date()).getTime() - n));

			//var n = (new Date()).getTime();
			var imgprov = new Crystal.SingleTileImageryProvider({
				url: d,
				rectangle: instance._rectangle
			});
			//console.log("Create imageryprovider: " + ((new Date()).getTime() - n));

			imgprov._tilingScheme = new Crystal.WebMercatorTilingScheme({
				rectangleSouthwestInMeters: new Crystal.Cartesian2(instance._mbounds.west, instance._mbounds.south),
				rectangleNortheastInMeters: new Crystal.Cartesian2(instance._mbounds.east, instance._mbounds.north)
			});

			return imgprov;
		};

		CrystalHeatmap._getID = function (len) {
			var text = "";
		...
		var WMP = new Crystal.WebMercatorProjection();

		/*  Convert a WGS84 location into a mercator location
		 *
		 *  p: the WGS84 location like {x: lon, y: lat}
		 */
		CrystalHeatmap.wgs84ToMercator = function (p) {
			var mp = WMP.project(Crystal.Cartographic.fromDegrees(p.x, p.y));
			return {
				x: mp.x,
				y: mp.y
			};
		 ...
		 *  bb: the WGS84 bounding box like {north, east, south, west}
		 */
		CrystalHeatmap.wgs84ToMercatorBB = function (bb) {
			var sw = WMP.project(Crystal.Cartographic.fromDegrees(bb.west, bb.south));
			var ne = WMP.project(Crystal.Cartographic.fromDegrees(bb.east, bb.north));
			return {
				north: ne.y,
				east: ne.x,
				south: sw.y,
		 ...
		 */
		CrystalHeatmap.mercatorToWgs84 = function (p) {
			var wp = WMP.unproject(new Crystal.Cartesian3(p.x, p.y));
			return {
				x: wp.longitude,
				y: wp.latitude
			};
		 ...
		 */
		CrystalHeatmap.mercatorToWgs84BB = function (bb) {
			var sw = WMP.unproject(new Crystal.Cartesian3(bb.west, bb.south));
			var ne = WMP.unproject(new Crystal.Cartesian3(bb.east, bb.north));
			return {
		 ...
		 */
		CrystalHeatmap.deg2rad = function (d) {
		 ...
		 */
		CrystalHeatmap.rad2deg = function (r) {
			var d = r / (Math.PI / 180.0);
			return d;
		};

		return CrystalHeatmap;
	}

	if (typeof(CrystalHeatmap) === 'undefined') {
		window.CrystalHeatmap = define_CrystalHeatmap();
	} else {
		console.log("CrystalHeatmap already defined.");
	}
})(window);

/*  Initiate a CrystalHeatmap instance
 *
 *  c:  CrystalWidget instance
 *  bb: a WGS84 bounding box like {north, east, south, west}
 *  o:  a heatmap.js options object
 */
function CHInstance(c, bb, o) {
	...
	this._cesium = c;
	this._options = o;
	this._id = CrystalHeatmap._getID();

	this._options.gradient = ((this._options.gradient) ? this._options.gradient : CrystalHeatmap.defaults.gradient);
	this._options.maxOpacity = ((this._options.maxOpacity) ? this._options.maxOpacity : CrystalHeatmap.defaults.maxOpacity);
	this._options.minOpacity = ((this._options.minOpacity) ? this._options.minOpacity : CrystalHeatmap.defaults.minOpacity);
	this._options.blur = ((this._options.blur) ? this._options.blur : CrystalHeatmap.defaults.blur);

	this._mbounds = CrystalHeatmap.wgs84ToMercatorBB(bb);
	this._setWidthAndHeight(this._mbounds);

	this._options.radius = Math.round((this._options.radius) ? this._options.radius : ((this.width > this.height) ? this.width / CrystalHeatmap.defaults.radiusFactor : this.height / CrystalHeatmap.defaults.radiusFactor));

	this._spacing = this._options.radius * CrystalHeatmap.defaults.spacingFactor;
	...
	this.bounds = CrystalHeatmap.mercatorToWgs84BB(this._mbounds);

	this._rectangle = Crystal.Rectangle.fromDegrees(this.bounds.west, this.bounds.south, this.bounds.east, this.bounds.north);
	this._container = CrystalHeatmap._getContainer(this.width, this.height, this._id);
...
CHInstance.prototype.wgs84PointToHeatmapPoint = function (p) {
	return this.mercatorPointToHeatmapPoint(CrystalHeatmap.wgs84ToMercator(p));
};
...
CHInstance.prototype._setWidthAndHeight = function (mbb) {
	...
	if (this.width > this.height && this.width > CrystalHeatmap.defaults.maxCanvasSize) {
		this._factor = this.width / CrystalHeatmap.defaults.maxCanvasSize;

		if (this.height / this._factor < CrystalHeatmap.defaults.minCanvasSize) {
			this._factor = this.height / CrystalHeatmap.defaults.minCanvasSize;
		}
	} else if (this.height > this.width && this.height > CrystalHeatmap.defaults.maxCanvasSize) {
		this._factor = this.height / CrystalHeatmap.defaults.maxCanvasSize;

		if (this.width / this._factor < CrystalHeatmap.defaults.minCanvasSize) {
			this._factor = this.width / CrystalHeatmap.defaults.minCanvasSize;
		}
	} else if (this.width < this.height && this.width < CrystalHeatmap.defaults.minCanvasSize) {
		this._factor = this.width / CrystalHeatmap.defaults.minCanvasSize;

		if (this.height / this._factor > CrystalHeatmap.defaults.maxCanvasSize) {
			this._factor = this.height / CrystalHeatmap.defaults.maxCanvasSize;
		}
	} else if (this.height < this.width && this.height < CrystalHeatmap.defaults.minCanvasSize) {
		this._factor = this.height / CrystalHeatmap.defaults.minCanvasSize;

		if (this.width / this._factor > CrystalHeatmap.defaults.maxCanvasSize) {
			this._factor = this.width / CrystalHeatmap.defaults.maxCanvasSize;
		}
	}
...
CHInstance.prototype.updateLayer = function () {
	...
	if (CrystalHeatmap.defaults.useEntitiesIfAvailable && this._cesium.entities) {
		if (this._layer) {
			this._cesium.entities.remove(this._layer);
		}

		// Work around issue with material rendering in Crystal
		// provided by https://github.com/criis
		material = new Crystal.ImageMaterialProperty({
			image: this._heatmap._renderer.canvas,
		});
		if (Crystal.VERSION >= "1.21") {
			material.transparent = true;
		} else if (Crystal.VERSION >= "1.16") {
			material.alpha = 0.99;
		}
		...
		this._layer = this._cesium.scene.imageryLayers.addImageryProvider(CrystalHeatmap._getImageryProvider(this));
	}
};
