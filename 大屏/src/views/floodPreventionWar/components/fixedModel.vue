 ...
 * @LastEditTime: 2021-08-28 17:53:57
-->
<template>
  <div class="c-fiexd-model">
    ...
    <div class="select-list-title-box">
      ...
      <div
        class="indetail"  v-if="indetailTitle.length!=0 "    v-show="activeKey || activeKey===0" >
        <div class="indetail_title">
          <ul>
            <li
              v-for="(e, i) in indetailTitle" :key="i" @click="clickLi(e, i)"
        ...
        <div class="indetail_content">
          <div class="water_search">
            <a-input-search placeholder="请输入名称"  @search="doSearch"  v-model="keywords"/>
          </div>
          <div class="table">
...
export default {
        ...
        },
 {
  ...
  methods: {
    ...
    getimg(name){
      return this.fixedListIcon.find(e=>e.name == name)?.icon ?? this.notFind;
    },
    // 图层规划数据结构
    getData() {
    ...
    doListActive(item, key, flag) {
      console.log('item-------------',item.interfaceCode);
      ...
      .indetail_title {
        ...
        ul {
          ...
          li {
            width: 100%;
            height: auto;
            border: 1px solid rgba(15, 48, 84, 0.6);
            cursor: pointer;
            padding: 5px;
            box-sizing: border-box;
            -o-box-sizing: border-box;
