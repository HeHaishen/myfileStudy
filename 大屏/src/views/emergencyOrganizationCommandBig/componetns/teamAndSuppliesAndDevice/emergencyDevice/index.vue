 ...
 * @LastEditTime: 2021-11-11 19:20:40
-->
<template>
  <div class="emergency_device">
    <div class="public-head">应急设备</div>
    <div class="public-content">
      <h2 class="carlist-head">保障设备</h2>
      <div class="carlist-box">
        <!-- 指挥车 -->
        <div
          class="carlist-item"
          v-for="(item, index) in carlist.filter((e) => e.equipType == '1')"
          :key="String(index)"
          @click="showCarBox(item)"
        >
          <div class="carlist-left">
            <img :src="imgObj[item.name]" />
          </div>
          <div class="carlist-right">
            <div class="carlist-title">{{ item.name }}</div>
            <div class="carlist-num">
              {{ item.num
              }}<span class="carlist-unit"> {{ item.unit || "辆" }}</span>
            </div>
          </div>
          <div
            class="selected"
            v-if="openMoreList.findIndex((e) => e.name == item.name) > -1"
          >
            <img
      ...
      <h2 class="carlist-head">通信设备</h2>
      <div class="carlist-box">
        <div
          class="carlist-item"
          v-for="(item, index) in carlist.filter((e) => e.equipType == '2')"
          :key="String(index)"
          @click="showCarBox(item)"
        >
          <div class="carlist-left">
        ...
        <!-- 800M -->
        <div class="carlist-item" @click="showCommunicationBox">
          <div class="carlist-left">
            <img
              src="../../../../../assets/images/majorFestivalsConductor/car-communication.png"
            />
          </div>
          <div class="carlist-right">
            <div class="carlist-title">800M通信</div>
            <div class="carlist-num">
              {{ communicationTotal }}<span class="carlist-unit"> 台</span>
            </div>
          </div>
...
export default {
  name: "",
  props: {},
  components: { CarUpView, CommunicationUpView },
  data() {
    return {
      carShow: false,
      defaultItem: {}, // 点击的指挥车
  ...
  methods: {
    ...
    carClose(record) {
      ...
      if (this.openMoreList.length > 0) {
        this.activeWindow =
          this.openMoreList[this.openMoreList.length - 1].name;
      }
    },
    // 打开800M弹窗
