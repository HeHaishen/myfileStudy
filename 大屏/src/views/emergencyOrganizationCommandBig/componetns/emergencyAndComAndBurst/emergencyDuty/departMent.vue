...
<template>
  <div class="depart-ment">
    ...
    <div class="depart-ment-content">
      <div class="title">
        领导：{{departMentObj.leader}}
      </div>
      <ul>
        <li v-for="(item,index) in departMentArr" :key='index'>
          <span class="leader-img">
            <img src="../../../../../assets/images/emergencyOrganizationCommand/leader_icon.png" alt="">
          </span>
          <div class="info">
            <p class="post">{{item.post}}</p>
            <p class="phone-message">{{item.name}}<span @click="clickMobilePhone(item, $event)">
                <img src="../../../../../assets/images/emergencyOrganizationCommand/phone_icon.png" alt=""></span><span @click="clickMessage(item, $event)"><img src="../../../../../assets/images/emergencyOrganizationCommand/message_icon.png" alt=""></span></p>
            <p>{{item.phone}}</p>
          </div>
        </li>
      </ul>
    ...
    doQuery() {
      ...
      getIdsmApi(code, data).then(res => {
        if (res.success) {
          ...
          const { dutyChief, dutyChiefTel, mainDuty, mainDutyTel, viceDuty, viceDutyTel, dutyLeader, dutyLeaderTel } = this.departMentObj || {}
