 ...
 * @LastEditTime: 2021-11-05 15:09:13
-->
<template>
  <div>
    <ModelBox :title="title" class="c-mybox-position" style="z-index: 8100">
      <template slot="content">
        <div class="content-all">
          <ul class="list">
            <li v-for="(i, k) in labelObj" :key="i.id" :class="[[4,5,6,7].includes(i.id)? 'li-gird':'']">
              <div class="li-label">
                <label for="">{{ i.label }}</label>
              </div>
              <div class="li-content">
                <div v-if="i.id !== 9 && i.id !== 8">{{ contentObj[i.contentKeys]||contentObj[i.contentKeys]==0 ? contentObj[i.contentKeys] : "-" }}</div>
                <div v-if="i.id == 8">{{ contentObj['fsyy'] ? contentObj['fsyy'] : contentObj['sjms']?contentObj['sjms']:'-'}}</div>
                ...
                <div v-if="i.id === 9">
                  ...
                  <p class="text-btn" v-for='(item,index) in similarArr' :key='item.rownum' @click="similarBtn(index,item)">
                    {{item.accTitile}}
                  </p>
                </div>
              </div>
...
export default {
  components: { ModelBox, EventDetail, similarEvent },
  props: {
    saveQuery: {
      type: Object,
      default: () => { { } }
  ...
  methods: {
    init() {
      this.doQuery(this.saveQuery.jbbh)
    },
    // 查询事件概览
    doQuery(sid) {
      let code = '8qddPGf8'
