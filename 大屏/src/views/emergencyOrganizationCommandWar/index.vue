 ...
 * @LastEditTime: 2021-11-10 14:21:16
-->
<template>
  <div class="major-wrap">
    ...
    <div class="major-contents">
      ...
      <!-- 左-start -->
      <!-- <ThematicAnalysis v-show="leftShow" /> -->
      <EventOverview v-show="leftShow" :saveQuery="saveQuery" />

      <EmergencyInfor v-show="leftShow" :eventData="saveQuery" />
      <!-- 左-END -->
      <!-- 右-start -->
      <AroundVideo ref="aroundVideo" v-show="rightShow" />
      <ProgressOf v-show="rightShow" :eventData="saveQuery" />
      <!-- 右-end -->
      <!-- <LeaderGroup /> -->
      <!-- 拨打电话 -->
      ...
      <!-- 最新 -->
      <DisasterAnalysis
        class="disaster_analysis"
        :eventData="saveQuery"
        :class="[!rightShow && 'hide']"
      />
      <!-- 悬浮-icon -->
      <SearchAddress @doSearch="searchAddress" />
      <!-- 清除标志点 -->
      ...
      <TestBox />
      <!-- 视频 -->
      <VideoBox :class="[rightShow ? '' : 'hidden-icon']" />
      <!-- 多人会议 -->
      <MeetingIcon :class="[rightShow ? '' : 'hidden-icon']" ref="MeetingIcon" />
      <!-- 通讯录 -->
      <ContactsIcon :class="[rightShow ? '' : 'hidden-icon']" />
      <!-- 无人机 -->
      <DroneIcon :class="[rightShow ? '' : 'hidden-icon']" />
      <!--计划案 -->
      <PlanIcon :class="[rightShow ? '' : 'hidden-icon']" ref="PlanIcon" />
      <!-- 事件 -->
      <EventIcon :class="[rightShow ? '' : 'hidden-icon']" />
    </div>
    <div
...
import {
  beginIntoEvent,
  addVideoGrounp,
} from "@/api/emergencyOrganizationCommandWar/index";
/*左侧组件*/
import EmergencyInfor from "./components/emergencyInfor/index.vue";
import ThematicAnalysis from "./components/thematicAnalysis/index.vue";
/*右侧组件*/
import AroundVideo from "./components/aroundVideo/index.vue";
import EventOverview from "./components/eventOverview/index.vue";
import ProgressOf from "./components/progressOf/index.vue";
/* 悬浮icon */
import SearchAddress from "./components/searchAddress";
import ClearMap from "./components/fixedIcon/clearMap/index.vue";
import DrawMap from "./components/fixedIcon/drawMap/index.vue";
import Zoom from "./components/fixedIcon/zoom/index.vue";
import DisasterAnalysis from "./components/disasterAnalysis/index.vue"; // 右下角三按钮
import Warning from "./components/fixedIcon/warning/index.vue"; //预警
import ChangeMapLayer from "./components/fixedIcon/changeMapLayer/index.vue"; //地图图层
import TestBox from "./components/fixedIcon/testBox/index.vue"; // 地图工具箱
import VideoBox from "./components/fixedIcon/videoBox/index.vue"; // 地图工具箱
import MeetingIcon from "./components/fixedIcon/meetingIcon/index.vue"; // 多人会议
import ContactsIcon from "./components/fixedIcon/contactIcon/index.vue"; // 通讯录
import DroneIcon from "./components/fixedIcon/droneIcon/index.vue"; // 通讯录
import PlanIcon from "./components/fixedIcon/planIcon/index.vue"; // 预案
import EventIcon from "./components/fixedIcon/eventIcon/index.vue"; // 通讯录
...
export default {
  ...
  created() {
    ...
    if (this.saveQuery.jbbh && this.saveQuery.jbbh.trim() !== "") {
      this.eventDetail = JSON.parse(JSON.stringify(this.$route.query));
      this.$nextTick(() => {
        this.getEventDetail(this.saveQuery.jbbh);
        this.doAddVideoGrounp();
      });
      this.doBeginIntoEvent(this.saveQuery);
    } else {
      this.$MyMessage.warning("无事件，政委你跳转到平时屏！");
      this.$router.push({ path: "/emergencyOrganizationCommand" });
    }
  },
  ...
  methods: {
      ...
      getIdsmApi(code, data)
        .then((res) => {
          if (res.success) {
            ...
            pGzznCore.pointLayerRemove({ type: "defaultTypesgEvent" });

            let datas = pGzznCore.deepClone(this.contentObj);
            let infos = [
              {
                title: "事件",
                textIndex: "xxbt",
            ...
            this.doDict(
              "ZHDP_EVENT_LEVEL",
              this.contentObj[this.labelObj[4].contentKeys]
            );
            this.doDict(
              "ZHDP_EVENT_TYPE",
              this.contentObj[this.labelObj[3].contentKeys]
            );
            //需要根据详情的数据获取相似的事件
            // this.getEvent()
            this.$nextTick(() => {
      ...
      getIdsmApi(code, data).then((res) => {
        if (res.success) {
          if (type == "ZHDP_EVENT_LEVEL") {
            ...
            this.eventDetail.sjLevel = res.body.data[0].dictValue;
          } else {
            this.eventDetail.sjType = res.body.data[0].dictValue;
    ...
    mapInit() {
      ...
      $(
        "button.crystal-button.crystal-toolbar-button[title='天地图影像']"
      ).hide();
      setTimeout(() => {
        var objs = {
    ...
    doAddVideoGrounp() {
      let sendObj = {
        eventId: this.saveQuery.jbbh,
        lat: this.saveQuery.wd,
        lon: this.saveQuery.jd,
        radius: 1.3,
      };
      addVideoGrounp(sendObj).then((res) => {
        if (res.success) {
          // this.$MyMessage.success("新建视频分组！");
          console.info('新建视频分组成功！')
        } else {
          // this.$MyMessage.error();
          console.warn('新建视频分组失败！')
        }
      });
    },
    doGeteQuipment() {},
