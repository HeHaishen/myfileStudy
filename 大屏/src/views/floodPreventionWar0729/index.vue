 ...
 * @LastEditTime: 2021-07-26 17:42:34
-->
<template>
  <div class="forest-fire"  style="background:#182734">
    ...
    <div class="forest-left">
          ...
          <div class="fire-targeting-msg">
			...
			<div class="targeting-msg-chart">
				<div class="msg-chart-wrap1">
					<div class="chart-show-number" v-show="targetingChose===2">{{sendThreeK}} </div>
					<div class="msg-chart-wrap2">
						<div class="chart-show-number" v-show="targetingChose===1">{{sendTwoK}}</div>
						<div class="msg-chart-wrap3">
							<div class="chart-show-number" v-show="targetingChose===0">{{sendOneK}}</div>
						</div>
					</div>
				</div>
			</div>
      ...
      <!--   <vue-seamless-scroll :data="nowTeamExpertList" :class-option="defaultOption" >
        ...
        <vue-seamless-scroll :data="nowTeamExpertChangeList" :class-option="defaultOption" >
          <li v-for="(item,index) in nowTeamExpertChangeList" :key="index">
            <div @click="teamExpertClick(item,index)">{{item.expertName}}</div>
	...
	<div class="forest-center" v-if="true">
		<div class="fire-model-title" style="padding-right:120px;">{{eventDatailObj['eventName']?eventDatailObj['eventName']:'广东省广州市石门国家森林公园发生火灾'}}</div>
		...
		<ul class="forest-center-list">
			<li>
				...
				<div class="center-list-detail">
					...
					<p>
						轻伤：<span  v-if="yearsStaitcsObj['minorWoundNum']">{{yearsStaitcsObj['minorWoundNum']?yearsStaitcsObj['minorWoundNum']:'0'}}</span>
					</p>
				</div>
			</li>
			<li>
          ...
          <p>
					<label>现场救援物资: </label>	<label v-if="yearsStaitcsObj && yearsStaitcsObj.sceneReliefMaterialsList && yearsStaitcsObj.sceneReliefMaterialsList.length" v-for="(item,index) in yearsStaitcsObj.sceneReliefMaterialsList" :key="index" v-show="index<2">{{item.materialName}}:<span>{{item.materialNum?item.materialNum:0}}</span>{{item.unit}},</label>
					</p>
          <p>
						<label>在途救援物资: </label><label v-if="yearsStaitcsObj && yearsStaitcsObj.waitReliefMaterialsList && yearsStaitcsObj.waitReliefMaterialsList.length" v-for="(item,index) in yearsStaitcsObj.waitReliefMaterialsList" :key="index" v-show="index<2">{{item.materialName}}:<span>{{item.materialNum?item.materialNum:0}}</span>{{item.unit}},</label>
					</p>
					<!-- <p>
  ...
  <div class="forest-right">
		<div class="forest-right-left">
			<div class="right-left-connection2">
				...
				<div class="left-connection2-wrap" v-if="tabActive===1">
					<ul>
						<li>
              <video src="../../assets/media/contact_point.mp4" muted autoplay loop style="height:100%;width:100%;"></video>
            </li>
        ...
        <div class="left-connection2-wrap" v-if="tabActive===0">
					<ul>
						<li>
              <img style="display:block;width:100%;height:100%" src="../../assets/images/fire_show1.png" alt="">
            </li>
						<li> 
               <img style="display:block;width:100%;height:100%" src="../../assets/images/fire_show2.png" alt="">
            </li>
					</ul>
				</div>
			</div>
			...
			<div class="right-left-uav2">
				...
				<div class="uav-list-wrap">
					<ul class="left-uav-list">
						<li>
							<div>
                 <video src="../../assets/media/video.mp4" muted autoplay loop style="height:100%;width:100%;"></video>
              </div>
						</li>
						<li>
							<div>
                 <video src="../../assets/media/video1.mp4" muted autoplay loop style="height:100%;width:100%;"></video>
              </div>
						</li>
						<li>
							<div></div>
				...
				<div class="uav-list-wrap">
					<ul class="left-uav-list">
						<li>
							<div>
                 <video src="../../assets/media/scan.mp4" muted autoplay loop style="height:100%;width:100%;"></video>
              </div>
						</li>
						<li>
							<div>
                 <video src="../../assets/media/scan2.mp4" muted autoplay loop style="height:100%;width:100%;"></video>
              </div>
						</li>
						<li>
							<div></div>
        ...
        <div class="right-right-content">
          <ul v-if="orderList.length">
            <li v-for="(item, index) in orderList" :key="index" :class="[index === 0?'first-msg-li':'']">
  ...
  <!-- <div class="forest-menu"></div> -->
	...
	<div class="flex-time-wrap">
        ...
        <div class="calibration">
          ...
          <div class="calibration-inner" v-for="(item, key) in timmerLineObj.nodeNum" :key="item" :style="item % 5 === 0?'height:5vh':''">
  ...
  <div class="right-menu" style="height:100%;width:186px;position:fixed;right:0;"><Menu></Menu></div>
...
import { getTeamInfo,getWeather,eventTaskSendDetail,yearsStaticsFire,getEventDetail,getArriveTeam} from "@/api/floodPreventionWar";
import {nowDisasterRelief, nowFireBrigade, nowTeamExpert,nowTeamExpertChange
 } from '@/api/forestFire'
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      if(this.$route.query && this.$route.query.isScale){ // 当路由带有isScale===true,触发缩放
        this.initScale()
      }
    });
  },
  ...
  methods: {
    ...
    getYearsStaitcs (callBack=()=>{}){
      yearsStaticsFire({body:{sid:'2521067874866184192'}}).then(res=>{
        if(res.success) {
          ...
          if(res.body && res.body['sceneReliefMaterialsList'] && res.body['sceneReliefMaterialsList'].length){
            console.log('ojbk')
            this.disasterReliefList.map((item,index)=>{
              let saveObj = null
    ...
    getEventTaskSendDetail (data = null) {
      ...
      const play=(size)=>{
        ...
        let showArr = that.orderList.length -1<0?0:that.orderList.length - 1
        let saveCurrentList = that.directiveList.slice(arr-showArr-size,arr-showArr)
    ...
    timeLineTimoutOut () {
      ...
      this.timeOut = setTimeout(()=>{
        ...
        pGzznCore.addPromiseUpdateTeamPath(this.currentMoveStep,function(vValue){
          // alert(44)
        ...
        if(this.msgSendType){
          this.sendOneK = this.sendOneK===0?0:this.sendOneK - parseInt(Math.random()*10)<0?0:this.sendOneK - parseInt(Math.random()*10)
          this.sendTwoK = this.sendTwoK===0?0:this.sendTwoK - parseInt(Math.random()*10)<0?0:this.sendTwoK - parseInt(Math.random()*10)
          this.sendThreeK =this.sendTwoK===0?0:this.sendThreeK - parseInt(Math.random()*10)<0?0:this.sendThreeK - parseInt(Math.random()*10)
        }
        clearTimeout(this.timeOut)
    ...
    getTeamList (arr) {
      getArriveTeam({body:arr}).then(res=>{
        if(res.success) {         
          ...
          // 救火年度统计
          this.getYearsStaitcs(()=>{
            let scenList =  JSON.parse(JSON.stringify(this.yearsStaitcsObj['sceneReliefMaterialsList']))  // 年度统计
            this.disasterReliefList.map((item,index)=>{
              let saveObj = null
