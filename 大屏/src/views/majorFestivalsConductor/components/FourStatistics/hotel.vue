 ...
 * @LastEditTime: 2021-10-16 12:58:36
-->
<template>
  <div class="Hotel">
    <pop-up-layer
      ...
      <template slot="content">
        <div class="hotelClass">
          ...
          <Clist class="list" :columns="columns" :listData="listData">
            ...
            <template slot="name" slot-scope="{ text, record }">
              <span :title="text"  :class="record.code<7&&record.code? 'lightHeight':''">
                {{ text }}
              </span>
            </template>
            <template slot="telephone" slot-scope="{ text, record }">
              <div :title="text" class="telephone" >
                <span :class="record.code<7&&record.code? 'lightHeight':''"> {{ text }}</span>
                <a-icon
                  type="phone"
                  class="icon"
        ...
        <div class="dw" @click="onHotelGPS(record)">
          ...
          <template slot="toVideo" slot-scope="{ record }">
             <span @click="toVideo(record)" class="toVideo" v-show="record.isVideo > 0"> <img src="../../../../assets/images/majorFestivalsConductor/video.svg" alt=""></span> 
      </template>
...
export default {
  ...
  methods: {
    toVideo(data){
      ...
      console.log(token,'-------------------->');
      window.open(`#/moreVideos?name=${data.name}&sid=${data.sid}&token=${token}`);
    ...
    getListData(type) {
      this.type =type
      let params = {
        body: { type: type, name: this.query.name, addr: this.query.addr ,code: this.getreplice(this.query.code) },
    ...
    doClose() {
      ...
      var objs = {
        ...
        imgUrlObj:{keys:"level",start:'gzznGIS/Image/gjh/sgtj4',end:'.png'},
        //type: "defaultType",
        type:"defaultTypesg"+this.type,
        layerName: "四馆",
        info:this.columns.filter((e) => e.textIndex != "dw" && e.textIndex != "code" && e.textIndex != "toVideo" && e.orgType != 2),
