 ...
 * @LastEditTime: 2021-10-12 14:39:31
-->
<!-- 救灾物资 -->
<template>
  <div class="manage">
        ...
        <clist class="list" :columns="columns" :listData="listData">
          ...
          >
            <span
              ...
              :title="text"
              :key="item.textIndex"
              @click="onChange(record, item.textIndex)"
            >
              {{ text }}
            </span>
          </template>
    ...
    <pop-up-layer
      ...
      @doClose="wzzlClose"
      opsition="pageCenter"
      :title="`${activeKey == 1 ? formData.mc : formData.qymc}-救灾物资`"
      ref="myPopUpLayer"
    >
      <div slot="content" class="upLayer_content">
...
export default {
  ...
  data() {
    return {
      columns1: [
        { title: "仓库名称", textIndex: "mc", width: "20%" },
        { title: "主管部门", textIndex: "zgdw", width: "25%" },
        { title: "储备总量", textIndex: "wzzl", width: "15%" },
        { title: "地址", textIndex: "dz", width: "30%" },
  ...
  methods: {
    doQuery() {
      ...
      let code = this.activeKey == 1 ? "lzVtiTyK" : "9naLnFkk";
    ...
    onGPS(record) {
      ...
      var objs = {
        imgUrl: "gzznGIS/Image/gjh/jzwz"+this.activeKey+".svg",
      ...
      this.activeKey == 1 ? "" : objs.phoneInfo = {phone:"fzrbgdh",name:"fzr"};
      items.lon = items.jd;
